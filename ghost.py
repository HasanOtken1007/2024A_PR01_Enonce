...
class Ghost:
    ...
    def move(self):
        ...
        if not self.dead:
            ...
            next_rect = pygame.Rect(next_x, next_y, GHOST_SIZE[0], GHOST_SIZE[1])# Créer un rectangle pour la prochaine position prévue
         ...
         #next_rect = pygame.Rect(next_x, next_y, GHOST_SIZE[0], GHOST_SIZE[1])
            ...
            if not self.check_collision(next_rect):      # TODO Vérifier si la prochaine position entre en collision avec un mur
                self.pos = (next_x, next_y)       # Utilisez `self.check_collision()` pour détecter si le fantôme va heurter un mur.
                self.rect = next_rect# TODO: Si aucune collision n'est détectée, mettre à jour la position du fantôme  
            else:
                self.change_direction()   # TODO: Changer la direction du fantôme s'il rencontre un mur                                          
    ...
    def change_direction(self):
        directions = [(-TILE_WIDTH, 0), (TILE_WIDTH, 0), (0, -TILE_HEIGHT), (0, TILE_HEIGHT)]# TODO: Créer une liste de toutes les directions possibles pour le fantôme (gauche, droite, haut, bas)

        random.shuffle(directions)# TODO: Mélanger aléatoirement les directions pour simuler un choix aléatoire avec `random.shuffle()`

        for direction in directions:  # TODO: Parcourir chaque direction et vérifier si elle est valide (pas de collision avec un mur)
            next_x = self.pos[0] + direction[0] *self.speed
            next_y = self.pos[1] + direction [1] * self.speed        # TODO: Calculer la prochaine position du fantôme en fonction de la direction

            
            next_rect = pygame.Rect(next_x, next_y, GHOST_SIZE[0], GHOST_SIZE[1])#ßCréer un rectangle représentant cette nouvelle position
            #next_rect = pygame.Rect(next_x, next_y, GHOST_SIZE[0], GHOST_SIZE[1])
            
            if not self.check_collision(next_rect):# TODO: Vérifier si cette direction entraîne une collision avec un mur en utilisant `self.check_collision()`
